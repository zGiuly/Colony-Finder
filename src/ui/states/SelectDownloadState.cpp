...
void SelectDownloadState::Render(AppController* controller)
{
    ...
    if (size1MonthVal < 0.0)
    {
        sprintf(btnText1Month, "Download Galaxy 1 Month (Querying...)");
    }
    else
    {
        double sizeGb = size1MonthVal / (1024.0 * 1024.0 * 1024.0);
        sprintf(btnText1Month, "Download Galaxy 1 Month (%.2f GB)", sizeGb);
    }
    ...
    if (sizeFullVal < 0.0)
    {
        sprintf(btnTextFull, "Download Full Galaxy (Querying...)");
    }
    else
    {
        double sizeGb = sizeFullVal / (1024.0 * 1024.0 * 1024.0);
        sprintf(btnTextFull, "Download Full Galaxy (%.2f GB)", sizeGb);
    }
    ...
    if (ImGui::Button("Scan & Verify Path", ImVec2(availWidthCol2, controller->GetButtonHeightMedium())))
    {
        ...
        {
            controller->SetErrorMessage("No spansh database file (galaxy.json.gz or galaxy_1month.json.gz) found in selected directory.");
